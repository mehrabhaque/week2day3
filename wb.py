...
# create a function called spot_diff?Spot
# compare the two strings
#check that they are different
# check where the dirrence occurs
# record the dirrecent in a list
def spot_diff(string_1, string_2):
    dif_location =[]
    for a in range(len(string_1)):
        if string_1[a] != string_2[a]:
            dif_location.append(a)
    return dif_location
